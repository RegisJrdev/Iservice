<template>
    <div class="w-full flex flex-col justify-center items-center p-4"> 
        <h1 class="mb-8"><strong class="text-xl">Categorias de Serviços</strong> <br> Escolha a categoria do serviço que você procura</h1>
        <div class="grid gap-8 xl:grid-cols-5 md:grid-cols-3 sm:grid-cols-2 grid-cols-1"> <!-- Adicionei grid-cols-1 para garantir 1 coluna em telas menores que sm -->
            <div v-for="categorie in Categorias" 
                 :key="categorie.name" 
                 class="w-full flex flex-col justify-center items-center p-4 h-40 shadow-md rounded duration-300 cursor-pointer hover:bg-gray-300 hover:scale-110 transition"> <!-- Removi w-50 e sm:w-full redundante -->
                <div
                ...
                <strong class="text-center">{{ categorie.name }}</strong> <!-- Adicionei text-center para centralizar o texto -->
            </div>
        </div>
    </div>
</template>
